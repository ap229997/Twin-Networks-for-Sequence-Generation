...
class ShowTellModel(nn.Module):
    ...
    def forward(self, fc_feats, att_feats, seq):
        ...
        for i in range(seq.size(1)):
            ...
            # temporarily
            #if i > 0:
            #    states.append(state[0].view(state[0].size()[1], state[0].size()[2])) # temporarily

        return torch.cat([_.unsqueeze(1) for _ in outputs[1:]], 1).contiguous()
        # temporarily
        # return torch.cat([_.unsqueeze(1) for _ in outputs[1:]], 1).contiguous(), torch.cat([_.unsqueeze(1) for _ in states], 1).contiguous()
